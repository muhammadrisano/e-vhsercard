            ...
            <div class="container-fluid py-1 px-3">
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb bg-transparent mb-0 pb-0 pt-1 px-0 me-sm-6 me-5">
                        <li class="breadcrumb-item text-sm"><a class="opacity-5 text-dark" href="javascript:;">Manager</a>
                        </li>
                        <li class="breadcrumb-item text-sm text-dark active text-white" aria-current="page">Detail Report Perminggu
                        </li>
                    </ol>
                    <h6 class="font-weight-bolder mb-0 text-white">Detail Report Perminggu</h6>
                </nav>
        ...
        <div class="container py-4" style="overflow-x: hidden">
            <div class="card">
                <div class="card-header">
                    <h6>Detail Report Perminggu</h6>
                    <a href="{{ url('laporan_minggu') }}" class="btn btn-warning">Kembali</a>
                    @if (session('status'))
                        ...
                        <table class="table align-items-center justify-content-center mb-0 diplay" id="myTable" style="width: 100%">
                          ...
                          <tbody>
                            ...
                            <tr>
                              ...
                              @php
                              $now = date('Y-m-d');
                                  $persentasi_user = DB::table('persentasi_proses')->where('user_id',$row->id)->where('mananger_id',Auth::user()->id)->where('expired_proses',$hari)->first();
                                  // @dd($kemarin_enam);
                              @endphp
                              <td>
                                @if ($persentasi_user)
                                <div class="progress-wrapper w-75 mx-auto">
                                  <div class="progress-info">
                                        <div class="progress-percentage">
                                          <span class="text-xs font-weight-bold">{{ $persentasi_user->status_proses }} %</span>
                                        </div>
                                      </div>
                                      <div class="progress">
                                        <div class="progress-bar bg-gradient-success w-{{ $persentasi_user->status_proses }}" role="progressbar" aria-valuenow="{{ $persentasi_user->status_proses }}" aria-valuemin="0" aria-valuemax="100"></div>
                                      </div>
                                    </div>
                                    @else
                                    <span class="badge bg-gradient-warning badge-pill">Belum Mengirim</span>
                                    @endif
                                </td>
                              <td class="align-middle">
                                <a href="{{ url('detail_perminggu_user_id' . '/' . $row->id . '/' . $hari) }}" class="btn btn-success">
