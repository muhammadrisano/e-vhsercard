    ...
    <div class="container-fluid py-1 px-3">
      <nav aria-label="breadcrumb">
        <ol class="breadcrumb bg-transparent mb-0 pb-0 pt-1 px-0 me-sm-6 me-5">
          ...
          <li class="breadcrumb-item text-sm text-dark active text-white" aria-current="page">Report Perhari</li>
        </ol>
        <h6 class="font-weight-bolder mb-0 text-white">Report Perhari</h6>
      </nav>
  ...
  <div class="container py-4" style="overflow-x: hidden">
    <div class="card">
      <div class="card-header">
        <h6>Report Perhari</h6>
        <a href="{{ url('jam_monitoring') }}" class="btn btn-warning">Kembali</a>
      @if (session('status'))
          ...
          <table class="table align-items-center justify-content-center mb-0 diplay" id="myTable" style="width: 100%">
            <thead>
              <tr>
                ...
                <th class="text-uppercase text-secondary text-xxs font-weight-bolder opacity-7 ps-2">Observasi</th>
              ...
              <tr>
                ...
                <td>
                  <div class="d-flex px-2">
                    <div class="my-auto">
                      {{-- @php
                          $result = DB::table('users')->leftJoin('persentasi_proses','persentasi_proses.user_id','=','users.id')->where('role',1)->where('mananger_user',Auth::user()->email)->where('email',$row->hse_nik)->first();
                      @endphp --}}
                      @if ($row->total_persen == 4)
                          <span class="badge bg-gradient-success badge-pill">Selesai</span>
                          @elseif($row->total_persen == null || !$row->total_persen)
                          <span class="badge bg-gradient-warning badge-pill">Belum Mengirim</span>
                          @else
                          <span class="badge bg-gradient-success badge-pill">{{ $row->total_persen }}</span>
                      @endif
                    </div>
                  </div>
                </td>
                <td class="align-middle">
                  <a href="{{ url('detail_observasi' . '/' . $row->id . '/' . $hari) }}" class="btn btn-success">
