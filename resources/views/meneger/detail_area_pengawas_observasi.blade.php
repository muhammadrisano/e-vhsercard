            ...
            <div class="modal-dialog modal-xl modal-dialog-centered" role="document">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                      <form action="{{ url('edit_manager' . '/' . Auth::user()->id) }}" method="POST" enctype="multipart/form-data">
                        @csrf
  ...
  <div class="container py-4" style="overflow-x: hidden">
    <div class="card">
      <div class="card-header">
        <a href="{{ url('detail_pengawas_manager_id' . '/' . $positions) }}" class="btn btn-warning">Kembali</a>
      @if (session('status'))
      ...
      <div class="card-body px-0 pt-0 pb-2">
        <div class="table-responsive p-4">
          <table class="table align-items-center justify-content-center mb-0 diplay" id="myTable" style="width: 100%">
            ...
            <tbody>
              @foreach ($area_pengawas as $result_data)
              @php
                // $result_datas = DB::table('hseobs')->where('hse_nik',$result_data->email)->where('event',$positions)->first();
                $users_akun = DB::table('users')->where('email',$result_data->hse_nik)->first();
                $positions_akun = DB::table('positions')->where('id_positions',$result_data->event)->first();
              @endphp
               <tr>
                                            ...
                                            <div class="d-flex px-2">
                                                <div class="my-auto">
                                                    <h6 class="mb-0 text-sm">
                                                        {{ date('d/m/Y', strtotime($result_data->dibuat_hseobs)) }}</h6>
                                                </div>
                                            </div>
                                        </td>
                                        <td>
                                            <div class="d-flex px-2">
                                                <div class="my-auto">
                                                    <h6 class="mb-0 text-sm">{{ $result_data->hse_name }}</h6>
                                                </div>
                                            </div>
                                        </td>
                                        <td>
                                            <div class="d-flex px-2">
                                                <div class="my-auto">
                                                    <h6 class="mb-0 text-sm">{{ $result_data->hse_nik }}</h6>
                                                </div>
                                            </div>
                                        </td>
                                        <td>
                                            <div class="d-flex px-2">
                                                <div class="my-auto">
                                                    <h6 class="mb-0 text-sm">{{ $result_data->mulai_kerja }}</h6>
                                                </div>
                                            </div>
                                        </td>
                                        <td>
                                            <div class="d-flex px-2">
                                                <div class="my-auto">
                                                    <h6 class="mb-0 text-sm">{{ $result_data->selesai_kerja }}</h6>
                                                </div>
                                            </div>
                                        </td>
                                        <td>
                                            <div class="d-flex px-2">
                                                <div class="my-auto">
                                                    <h6 class="mb-0 text-sm">{{ $result_data->cabang }}</h6>
                                                </div>
                                            </div>
                                        </td>
                                        <td>
                ...
                <td class="align-middle">
                  <a href="#" class="btn btn-success" data-bs-toggle="modal" data-bs-target="#detail_form{{ $result_data->id_hseobs }}">
                    <i class="fas fa-eye"></i>
                  </a>
                  <a href="{{ asset('assets/img' . '/' . $result_data->img_hseobs) }}" download="" class="btn btn-info">
                    <i class="fas fa-download"></i>
                  </a>
                  <div class="modal fade" id="detail_form{{ $result_data->id_hseobs }}" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
                                  aria-hidden="true">
                                  <div class="modal-dialog modal-xl modal-dialog-centered" role="document">
                                      <div class="modal-content">
                                          <div class="modal-header">
                                          ...
                                          <div class="modal-body">
                                            <ul class="list-group">
                                                <li class="list-group-item">Cabang : {{ $result_data->cabang }}</li>
                                                <li class="list-group-item">Nama Area : {{ $positions_akun->positions }}</li>
                                                <li class="list-group-item">Nik : {{ $result_data->hse_nik }}</li>
                                                <li class="list-group-item">Nama : {{ $result_data->hse_name }}</li>
                                                <li class="list-group-item">Jam Mulai Observasi : {{ $result_data->mulai_kerja }}</li>
                                                <li class="list-group-item">Lokasi Kejadian : {{ $result_data->location }}</li>
                                                <li class="list-group-item">Kondisi : {{ $result_data->kondisi_observasi }}</li>
                                                <li class="list-group-item">Type Aktifitas yang Diamati :{{ $result_data->type_activity }}</li>
                                                <li class="list-group-item">Apa Kejadian yang Diamati : {{ $result_data->amati }}</li>
                                                <li class="list-group-item">Tindakan Perbaikan yang Dilakukan : {{ $result_data->perbaikan }}</li>
                                                <li class="list-group-item">Apa yang harus diLakukan Supaya Kejadian Serupa Tidak Terulang : {{ $result_data->terulang }}</li>
                                                <li class="list-group-item">Bukti Kondisi : 
                                                    <div style="width: 200px;height: 150px;">
                                                        <img src="{{ asset('assets/img' . '/' . $result_data->img_hseobs) }}" style="width: 100%;height: 100%;" alt="">
                                                    </div>
                                                </li>
                                              <li class="list-group-item">Jam Selesai Observasi: {{ $result_data->selesai_kerja }}</li>
                                              ...
                                              @php
                                                  // real time
                                                  $akun = DB::table('users')
                                                      ->where('email', $result_data->hse_nik)
                                                      ->first();
                                                  $real_time = DB::table('real_time_observasi')
                                                      ->where('hseobs_id', $result_data->id_hseobs)
                                              ...
                                              <li class="list-group-item">Corretive User Mananger :
                                                @if ($result_data->coretive == null)
                                                    Belum Di Isi
                                                @else
                                                    <p>{{ $result_data->coretive }}</p>
                                                @endif
                                            </li>
                                            <li class="list-group-item">Corretive User Hse Audit :
                                                @if ($result_data->approve_hse_audit == null)
                                                    Belum Di Isi
                                                @else
                                                    <p>{{ $result_data->approve_hse_audit }}</p>
                                                @endif
                                            </li>
                                          </ul>
                                          </div>
