...
<main class="main-content position-relative max-height-vh-100 h-100 mt-1 border-radius-lg ">
  <!-- Navbar -->
  <nav class="navbar navbar-main navbar-expand-lg px-0 bg-gradient-danger mx-4 shadow-none border-radius-xl" id="navbarBlur" navbar-scroll="true">
    <div class="container-fluid py-1 px-3">
      <nav aria-label="breadcrumb">
        <ol class="breadcrumb bg-transparent mb-0 pb-0 pt-1 px-0 me-sm-6 me-5">
          <li class="breadcrumb-item text-sm"><a class="opacity-5 text-dark" href="javascript:;">Manager</a></li>
          <li class="breadcrumb-item text-sm text-dark text-white active" aria-current="page">Observasi</li>
        </ol>
  ...
  <div class="container py-4" style="overflow-x: hidden">
            ...
            <tbody>
              ...
              <tr>
                ...
                <td class="align-middle">
                  <a href="#" class="btn btn-success" data-bs-toggle="modal" data-bs-target="#detail_form{{ $row->id_hseobs }}">
                    <i class="fas fa-eye"></i>
                  </a>
                  <div class="modal fade" id="detail_form{{ $row->id_hseobs }}" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
                                  aria-hidden="true">
                                  <div class="modal-dialog modal-xl modal-dialog-centered" role="document">
                                      <div class="modal-content">
                                          <div class="modal-header">
                  ...
                  {{-- <a href="{{ url('hapus_observasi_manager' . '/' . $row->id_hseobs) }}" class="btn btn-danger">
                    <i class="fas fa-trash"></i>
                  </a> --}}
                  @if ($row->conditions == null)
                  <a href="#" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#setuju{{ $row->id_hseobs }}">
                    <i class="fas fa-check"></i>
                  </a>
                  <a href="#" class="btn btn-warning" data-bs-toggle="modal" data-bs-target="#id{{ $row->id_hseobs }}">
                    <i class="fas fa-times"></i>
                  </a>
                  <div class="modal fade" id="setuju{{ $row->id_hseobs }}" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
                    aria-hidden="true">
                    <div class="modal-dialog modal-xl modal-dialog-centered" role="document">
                        <div class="modal-content">
                            <div class="modal-header">
                                <h5 class="modal-title" id="exampleModalLabel">Setuju Observasi</h5>
                            </div>
                            <div class="modal-body">
                              <form action="{{ url('terima_observasi' . '/' . $row->id_hseobs) }}" method="POST" enctype="multipart/form-data">
                                @csrf
                              <div class="form-group">
                                <label for="pesan" class="form-control-label">Pesan</label>
                                <textarea class="form-control @error('pesan')
                                is-invalid
                                @enderror"  value="{{ old('pesan') }}" id="pesan" name="pesan" required>{!! $row->coretive !!}</textarea>
                              </div>
                                <button type="submit" class="btn btn-success">Simpan</button>
                              </form>
                            </div>
                  ...
                  <div class="modal fade" id="id{{ $row->id_hseobs }}" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
                    ...
                    <div class="modal-dialog modal-xl modal-dialog-centered" role="document">
                        <div class="modal-content">
                            <div class="modal-header">
                                <h5 class="modal-title" id="exampleModalLabel">Tolak Observasi</h5>
                            </div>
                            <div class="modal-body">
                              <form action="{{ url('edit_observasi_tolak' . '/' . $row->id_hseobs) }}" method="POST" enctype="multipart/form-data">
                                @csrf
                              <div class="form-group">
                                <label for="pesan" class="form-control-label">Pesan</label>
                                <textarea class="form-control @error('pesan')
    ...
    <div class="card mt-5">
      <div class="card-header">
        <h6>Submission</h6>
        @if ($statistik_count)  
        <form action="{{ url('cari_mananger') }}" method="POST" enctype="multipart/form-data">
        @csrf
        <div class="row">
          <div class="col-md-6 mt-1">
            <div class="input-groups">
              <label for="waktu">Waktu</label>
              <input type="date" name="waktu" class="form-control @error('waktu')
                is-invalid
              @enderror" autocomplete="waktu"  required>
            </div>
            <div class="input-groups">
              <label for="area">Area</label>
              <select name="area" class="form-control @error('area')
                is-invalid
              @enderror" autocomplete="area"  required>
              @foreach ($area as $areas)
                <option value="{{ $areas->positions }}">{{ $areas->positions }}</option>
              @endforeach
              </select>
            </div>
          </div>
      ...
      <div class="card-body">
        ...
        <div id="chart"></div>
        @else
        <div class="alert alert-danger mt-3 text-white" role="alert">Data Tidak Ada</div>
        @endif
      </div>
    </div>
    <div class="card mt-5">
      <div class="card-header">
        <h5>Aktifitas Sedang Berjalan</h5>
      </div>
      <div class="modal-body">
        @php
            // real time
            ...
            $real_time = DB::table('real_time_observasi')
                ->get();
            $real_times = DB::table('real_time_observasi')->where('mananger_id_user',Auth::user()->email)
                ->count();
        @endphp
        @if ($real_times)
...
<script>
  var options = {
          series: [
        {
          ...
          data: [
            @php
              foreach($statistik as $roy) {
                echo "' $roy->score_submission_satu '" . ',';
              }
            @endphp
            ]
        },
        ...
        },
        {
          ...
          data: [
            @php
              foreach($statistik as $roy) {
                echo "' $roy->score_submission_tiga '" . ',';
              }
            @endphp
            ]
        },
        {
          ...
          data: [
            @php
              foreach($statistik as $roy) {
                echo "' $roy->score_submission_empat '" . ',';
              }
            @endphp
            ]
        },
        {
          name: 'Nearmiss/nyaris celaka',
          data: [
            @php
              foreach($statistik as $roy) {
                echo "' $roy->score_submission_lima '" . ',';
              }
            @endphp
            ]
        },
        {
          ...
          data: [
            @php
              foreach($statistik as $roy) {
                echo "' $roy->score_submission_enam '" . ',';
              }
            @endphp
            ]
        },
        ...
        xaxis: {
          ...
          categories: [
            ...
            @php
              foreach($statistik as $roy) {
                echo "' $roy->type_aktifitas_submission '" . ',';
              }
            @endphp
          ],
        },
