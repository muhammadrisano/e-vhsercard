  ...
  <div class="container py-4" style="overflow-x: hidden">
                  ...
                  <div class="modal fade" id="setuju{{ $row->id_hseobs }}" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
                    ...
                    <div class="modal-dialog modal-xl modal-dialog-centered" role="document">
                        <div class="modal-content">
                            ...
                            <div class="modal-body">
                              <form action="{{ url('terimas_observasi' . '/' . $row->id_hseobs) }}" method="POST" enctype="multipart/form-data">
                                @csrf
                              <div class="form-group">
                                <label for="pesan" class="form-control-label">Pesan</label>
                                <textarea class="form-control @error('pesan')
                                is-invalid
                                @enderror"  value="{{ old('pesan') }}" id="pesan" name="pesan" required>{{ $row->approve_hse_audit }}</textarea>
                              </div>
                                <button type="submit" class="btn btn-success">Simpan</button>
                              </form>
                            </div>
    ...
    <div class="card mt-5">
      <div class="card-header">
        <h6>Submission</h6>
        @if ($statistik_count)
        <form action="{{ url('cari_hse') }}" method="POST" enctype="multipart/form-data">
          @csrf
          <div class="row">
            <div class="col-md-6 mt-1">
    ...
    <div class="card mt-5">
      ...
      <div class="modal-body">
        @php
            ...
            $real_time = DB::table('real_time_observasi')
                ->get();
            $real_time_count = DB::table('real_time_observasi')
                ->count();
        @endphp
        @if ($real_time_count)  
